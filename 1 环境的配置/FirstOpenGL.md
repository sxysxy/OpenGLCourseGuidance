...
至此OpenGL开发环境安装完成，可以试验一下，创建一些新的C++项目，加入下面的代码（或者直接下载教程的FirstOpenGL工程）

目前阶段不必理解代码的含义，这只是为了测试你刚才的OpenGL环境是否成功安装，代码有意地使用了现代OpenGL的特性去实现，如果代码能够顺利编译运行看到金色分形三角形，那么恭喜你，可以正式开始计算机图形学和OpenGL的学习了。
