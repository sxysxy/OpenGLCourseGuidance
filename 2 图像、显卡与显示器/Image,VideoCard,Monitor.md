...
&emsp;&emsp;首先，由于计算机显示器是发光的光源，因此显示的任何颜色都是通过红色(R)，绿色(G)，蓝色(B)所谓的光学三原色合成所得。图形学中也常常使用三原色的信息去表示图像，一个颜色可以用向量(R, G, B)表示。每个分量的范围，如果用整数表示的话常见范围是\[0, 255\](只取整数)，如果用实数表示，则常见范围为\[0, 1\](包含0到1之间所有实数)。OpenGL中常用float类型(单精度(32位)浮点数类型)表示颜色的分量，颜色的分量的取值就是在\[0, 1\]之间的实数。一个分量的值越大，则其对应的颜色越“浓”。如果使用整数表示颜色的分量，那么纯红色就是(255, 0, 0)，纯绿色是(0, 255, 0)，纯蓝色是(0, 0, 255)，纯紫色是纯红色与纯蓝色的混合，是(255, 0, 255)。此外，还可以给颜色加入第四个分量，不透明度A(Alpha)，通常用在多张图像的混合中，直观地看，不透明度越大，则在与其它图像叠加合成时，越无法看到在这张图片的下面的图像。(有使用过Photoshop的同学都懂的~)
...
&emsp;&emsp;每一个像素点上通常储存这一个像素的颜色信息，颜色信息数据的大小(比特数)称作**位深度**，例如图像每一个像素使用3个unsigned char储存颜色信息，那么图像的位深度为3x8=24(每个unsigned char有8个比特，3个有24比特)。 最终图像的有像素构成了图像。
...
1111
```
查颜色表可得，1表示(0,0,255), 0表示(255,0,0)，那么这就看作是一张背景为蓝色，中间一条红线的图像。
...
<li> 如果你的电脑支持并设置为1080P(1920x1080)分辨率，刷新率为120Hz，24位真彩色模式(位深度为24)，那么理论上对你的显卡到显示器的带宽的最低要求是多大？（题库中有类似题目）
